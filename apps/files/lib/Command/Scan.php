...
class Scan extends Base {
		...
		$scanner->listen('\OC\Files\Utils\Scanner', 'scanFile', function (string $path) use ($output, $scanMetadata) {
			...
			if ($scanMetadata) {
				...
				$this->filesMetadataManager->refreshMetadata(
					$node,
					($scanMetadata !== '_LIVE_') ? IFilesMetadataManager::PROCESS_NAMED : IFilesMetadataManager::PROCESS_LIVE | IFilesMetadataManager::PROCESS_BACKGROUND,
					$scanMetadata
				);
			}
		});
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if ($input->hasOption('generate-metadata')) {
			$metadata = $input->getOption('generate-metadata') ?? '_LIVE_';
		}
